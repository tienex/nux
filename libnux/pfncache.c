...
/*
 * We need a pfncache to enable pfnalloc, which in turns enable the kmem.
 * 
 * Before the kmem starts, we can't reserve the kmem space needed to hold all
 * the slots. We start with a single, static entry in the pfncache.
 * 
 * Once kmem is setup _nux_init() will call pfncacheinit(), that will reserve
 * the required amount of slots and start the real, full size cache.
 */
