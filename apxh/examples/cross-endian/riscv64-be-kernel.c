 ...
 *
 * Demonstrates cross-endian boot where a little-endian bootloader
 * boots a big-endian RISC-V kernel. Verifies that boot structures
 * are correctly byte-swapped.
 *
 * Build with:
 *   riscv64-linux-gnu-gcc -march=rv64imafdc -mabi=lp64d -mbig-endian \
 *     -nostdlib -static -Wl,-T,riscv64-be.lds -o riscv64-be-kernel.elf \
 *     riscv64-be-kernel.c riscv64-start.S
 */
...
typedef struct {
    ...
    uint8_t  KernelEndianness;      // ImgEndianBig = 2
...
// Get native endianness (should always be big-endian for this kernel)
...
static int verify_endianness(void) {
    ...
    if (gBootInfo->KernelEndianness == ImgEndianBig) {
        uart_puts(" (big-endian) [OK]\n");
    } else {
        uart_puts(" [FAIL - expected big-endian]\n");
        return 0;
    }
...
void kernel_main(uint64_t boot_info_addr, uint64_t regions_addr) {
    ...
    uart_puts("\n");
    uart_puts("========================================\n");
    uart_puts("RISC-V64 Big-Endian Test Kernel\n");
    uart_puts("========================================\n");
    uart_puts("\n");

    uart_puts("Testing cross-endian boot scenario:\n");
    uart_puts("  Bootloader: little-endian (assumed)\n");
    uart_puts("  Kernel:     big-endian (this kernel)\n");
    uart_puts("\n");
